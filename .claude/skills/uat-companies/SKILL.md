...
name: uat-companies
description: >
  Run a UAT of company discovery in managed mode: login, upload startup resume,
  store Gemini key, discover 5 companies with "Startups" focus, and verify the
  startups badge appears on the run.
  Use this skill whenever the user says "run uat-companies", "test company discovery",
  "test startup discovery", or any similar phrase about testing the company discovery flow.
---

# UAT — Company Discovery (Managed Mode)

Tests the company discovery flow end-to-end: login, resume upload, API key setup,
...
## Step 4 — Upload startup resume

1. `preview_snapshot` to confirm the ResumeTab is visible
...
1. Read the key from env: `source ~/.env && echo $GEMINI_API_KEY`
2. Click profile menu: `preview_eval` → `document.querySelector('[aria-label="Profile menu"]').click()`
3. Wait ~500ms, then click "API Keys" menu item
4. Wait ~500ms, then `preview_snapshot` to confirm ApiKeysDialog
5. If Gemini already shows "Stored", skip to step 9
6. Fill the Gemini key input and click Save
7. Wait ~3 seconds for validation
8. `preview_snapshot` — **verify**: "Stored" badge next to Gemini
9. Close the dialog
10. `preview_screenshot` as proof

## Step 6 — Discover 5 companies with "Startups" focus
